...
export default function HRComplaints() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError(null);
        const res = await fetchComplaints({ recipient: 'HR' });
        if (!cancelled) {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-surface-text">{t('HR Complaint Inbox')}</h1>
        <p className="text-sm text-surface-muted mt-1">{t('Complaints routed to HR for review and resolution.')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="professional-card p-12 text-center">
          ...
          <p className="text-sm text-surface-muted max-w-md mx-auto">{t('No HR complaints have been submitted yet. Complaints filed by employees will appear here.')}</p>
        </div>
      ) : (
        /* Complaints List */
        <div className="space-y-4">
